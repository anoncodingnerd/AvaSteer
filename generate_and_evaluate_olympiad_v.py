...
_BOX_TAG_RE = re.compile(r"<\|begin_of_box\|>(.*?)<\|end_of_box\|>", re.DOTALL)
...
def extract_boxed_content(text: str) -> str:
    ...
    if "<|begin_of_box|>" in text and "<|end_of_box|>" in text:
        ...
        try:
            start = text.rfind("<|begin_of_box|>")
            if start != -1:
                i = start + len("<|begin_of_box|>")
                end = text.find("<|end_of_box|>", i)
                if end != -1:
...
def extract_last_latex(text: str) -> str:
    if not isinstance(text, str):
        return "None"
    matches = re.findall(r"\$\$([^$]+)\$\$|\$([^$]+)\$", text)
    for m1, m2 in reversed(matches):
        val = (m1 or m2).strip()
        if val:
            return val
    return "None"
...
def strip_outer_dollars(text: str) -> str:
    if not isinstance(text, str):
        return ""
    t = text.strip()
...
def generate_and_evaluate(
    ...
    dataname="OlympiadBenchZhComp",
    data_path="Data/exports/OlympiadBenchZhComp/OlympiadBench.jsonl",
    base_save_path="",
    ...
    images_root="",
    outputs_dir="",
    run_name="Qwen3VL_OlympiadBench",
    enforce_eager=False,
):
    from vllm import LLM, SamplingParams, ModelRegistry
    from steer_qwen3_vl_vllm import SteerQwen3VLForConditionalGeneration
    # from steer_qwen3_vl_vllm_no_calibration import SteerQwen3VLForConditionalGeneration

    if not data_path:
        raise ValueError("data_path is required.")

    if not base_save_path:
        base_save_path = os.path.join(os.path.dirname(data_path), "outputs")
    if not os.path.exists(base_save_path):
        os.makedirs(base_save_path, exist_ok=True)
    if not generation_save_path:
        generation_save_path = os.path.join(base_save_path, f"{run_name}.json")
    if not overall_trend_save_path:
        overall_trend_save_path = os.path.join(base_save_path, f"{run_name}_trend.json")
    ...
    if steering_enabled:
        if "Qwen" in model_path:
            ModelRegistry.register_model(
                ...
                #"steer_qwen3_vl_vllm_no_calibration:SteerQwen3VLForConditionalGeneration",
            )
            print("Registered SteerQwen3VLForConditionalGeneration for vLLM.")
        else:
            ModelRegistry.register_model(
                "Glm4vForConditionalGeneration",
                "steer_glm4_1v_vllm:SteerGlm4vForConditionalGeneration",
            )
            print("Registered SteerGlm4vForConditionalGeneration for vLLM.")
    else:
    ...
    def create_llm(steering_strength, steering_vector_path, model_path, tensor_parallel_size=1):
        ...
        if steering_enabled:
            ...
            num_layers = getattr(getattr(config, "text_config", config), "num_hidden_layers", getattr(config, "num_hidden_layers", 1))
            ...
            for idx in (7, 8, 9, 10, 11):
                steering_strength_list[idx] = base_strength
    ...
    if steering_enabled:
        try:
            ...
            print(f"[steer] vLLM model_info.architecture={model_arch} using_transformers_backend={using_tf_backend}")
    ...
    def _count_image_placeholders(prompt_text: str) -> int:
    ...
    for datapoint in tqdm(question_dataset):
        raw_question = datapoint.get("question") or datapoint.get("problem") or ""
        question = clean_question(raw_question)
        context = datapoint.get("context")
        if isinstance(context, str) and context.strip():
            question = f"{context.strip()}\\n\\n{question}"

        prompt_temp = prompt_template.render(prompt=question, options_block="")

        system_content = "请逐步推理，并将最终答案写在\\boxed{}中。"
        ...
        placeholder_count = _count_image_placeholders(prompt_text)
    ...
    for i in range(len(all_generated_texts)):
        ...
        raw_ans = datapoint.get("final_answer")
        if raw_ans is None:
            raw_ans = datapoint.get("answer")
        ...
        gt_candidates.extend([strip_outer_dollars(c) for c in gt_candidates if c])
        ...
        for rollout in one_generation:
            ...
            total_output_tokens += llm_reasoning_token_num + llm_answer_token_num

            is_correct = mathv_equal(strip_outer_dollars(llm_final_answer), gt_candidates)
    ...
    avg_latency_s = generation_seconds / total if total else 0.0
    print(f"Average time latency (s): {avg_latency_s}")
    output_tokens_per_sec = total_output_tokens / generation_seconds if generation_seconds else 0.0
    print(f"Output tokens/s: {output_tokens_per_sec}")
    ...
    new_entry = {
        ...
        "average_time_latency_s": avg_latency_s,
