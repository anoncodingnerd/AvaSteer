...
from vllm.model_executor.models.glm4 import Glm4DecoderLayer
...
class SteerGlm4DecoderLayer(Glm4DecoderLayer):
    def __init__(
        self,
        ...
        self.register_buffer("steering_strength", None)
        ...
        self.adaptive_steering = False
        self._set_adaptive_cfg({})
    ...
    def _set_adaptive_cfg(self, cfg: dict[str, float | str | None]) -> None:
        ...
        # Store as tensor buffer to stay fully tensorized under torch.compile.
    ...
    def _adaptive_strength(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Keep eps tensorized for torch.compile.
...
class SteerGlm4Model(LlamaModel):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__(
            vllm_config=vllm_config, prefix=prefix, layer_type=SteerGlm4DecoderLayer
        )

        steering_vector_path = os.environ.get("steering_vector_path")
        steering_strength_str = os.environ.get("steering_strength_list")
        if not steering_vector_path or not steering_strength_str:
            return
        ...
        if calibration_vectors is not None:
            if calibration_vectors.shape[0] == num_layers + 1:
                print(
                    "[warn] calibration vectors include embedding layer; dropping last layer.",
                    flush=True,
                )
        ...
        for layer_idx, layer in enumerate(self.layers):
            if isinstance(layer, SteerGlm4DecoderLayer):
...
class SteerGlm4ForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
...
try:
    ...
    ModelRegistry.register_model("SteerGlm4ForCausalLM", SteerGlm4ForCausalLM)
except Exception as exc:
    logger.warning("Failed to register SteerGlm4ForCausalLM: %s", exc)
...
class SteerGlm4vForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        use_steering = bool(os.environ.get("steering_vector_path")) and bool(
            os.environ.get("steering_strength_list")
        )
        if config.model_type == "glm4v":
            architectures = (
                ["SteerGlm4ForCausalLM"] if use_steering else ["Glm4ForCausalLM"]
            )
        elif config.model_type == "glm4v_moe":
            if use_steering:
                logger.warning(
                    "Adaptive steering not supported for glm4v_moe; using Glm4MoeForCausalLM."
                )
...
class Glm4vMoeForConditionalGeneration(SteerGlm4vForConditionalGeneration):
    packed_modules_mapping = {
        "qkv_proj": [
            "q_proj",
            "k_proj",
